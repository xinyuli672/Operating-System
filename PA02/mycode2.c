	...
	int n;				// percent of CPU time
{
  ...
  if(n>=0 && n<=100){
    if(n == 0){
        ...
        DPrintf("\nsetting stride to 0 because n = 0? n=%d  stride = %d\n", n,proctab[index].stride);
    }
    for(int i = 0; i < MAXPROCS; i++){
     if(proctab[i].valid){
       if(proctab[i].pid == p){
    ...
    DPrintf("\ncheck if allocation , counting n, will exceed 100% cpu usage : before cpu = %d  req cpu= %d  pid %d\n",sum,n,p);
    sum = sum + n;
    if(sum <= 100){
      proctab[index].req = n;
      proctab[index].cpu = n;
      proctab[index].stride = 100000 / n;
      DPrintf("\nsetting stride to %d\n",proctab[index].stride);
    }
       
//      unused_cpu = 100 - sum;
//      //loop through the processes table, count how many processes are with req ==0
//      for(int i = 0; i < MAXPROCS; i++){
//       if(proctab[i].valid && proctab[i].req == 0) count++;  
//      }
//    }
//      //if there's any processes with req == 0, distribute unused cpu evenly to them 
//      if(count != 0){
//        if(unused_cpu < count){
//          share = unused_cpu;make
////          DPrintf("\n share is small = \n",share);
////          for(int s = 0; s < MAXPROCS; s++){
////           if(proctab[s].valid && proctab[s].req == 0){  //look for the process with min cpu
////             if(!flag || proctab[s].cpu < min) {
////                 proctab[s].cpu = min;
////                 temp = s;
////             }         
////           }
////          }
////
////          proctab[temp].cpu = share;
////          proctab[temp].stride = 100000 / share;
////          
////          for(int k = 0; k < MAXPROCS; k++){
////            if(proctab[k].valid && proctab[k].req == 0 && k != temp)
////              proctab[k].cpu = 0;
////              proctab[k].stride = 0;
////          }
//        }else{
